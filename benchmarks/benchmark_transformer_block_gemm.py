"""文件职责：比较纯 PyTorch Block 与只替换 GEMM 后的端到端延迟。"""
...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "results" / "transformer_block_gemm.csv"
...
def load_transformer_block_module():
    ...
    spec = importlib.util.spec_from_file_location("operator_engine_transformer_block_gemm_benchmark", module_path)
...
def main() -> None:
    ...
    for case_index, case_name in enumerate(selected_cases):
        ...
        gemm_block.load_state_dict(baseline.state_dict())
        ...
        with torch.inference_mode():
            ...
            actual = gemm_block(hidden_states)
        ...
        for provider, latency_ms in latencies.items():
            expected_gemm_path = "pytorch_linear" if provider == "torch_baseline" else "v1_tiled_transpose_b"
            speedup_vs_torch = latencies["torch_baseline"] / latency_ms
            rows.append({"case": case_name, "B": batch_size, "N": sequence_length, "hidden_size": hidden_size, "num_heads": num_heads, "head_dim": head_dim, "intermediate_size": intermediate_size, "provider": provider, "expected_gemm_path": expected_gemm_path, "latency_ms": latency_ms, "speedup_vs_torch": speedup_vs_torch})
            print(f"{case_name:<22} B={batch_size:>2} N={sequence_length:>4} hidden={hidden_size:>4} H={num_heads:>2} D={head_dim:>3} {provider:<15} {expected_gemm_path:<22} {latency_ms:8.4f} ms {speedup_vs_torch:7.3f}x")

        print(f"{'':22} GEMM-only block speedup: {latencies['torch_baseline'] / latencies['gemm_only']:.3f}x")
